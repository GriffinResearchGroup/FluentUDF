...
DEFINE_SDOF_PROPERTIES(UAVAJ, prop, dt, time, dtime)
{
FILE* fp;
...
DEFINE_CG_MOTION(Follower_Fluid_UAVAJ,dt,vel,omega,time,dtime)
{
NV_S (vel, = , 0.0);
NV_S (omega, = , 0.0);
vel[0]=0;
...
DEFINE_SDOF_PROPERTIES(HH, prop, dt, time, dtime)
{
   prop[SDOF_MASS]       = 330.6276009;
   prop[SDOF_IXX]        = 29.41645817;
   prop[SDOF_IYY]        = 767.7691132;
   ...
   if (time <= 0.0471)
   {
      Sep_Fx1 = -(-130470150184*pow(time,5) + 20694827653.7812*pow(time,4) - 1229644001.3096*pow(time,3) + 30156210.2186*pow(time,2) - 30145.5238*time - 11633.9948);
      Sep_Mz1 = (-16269627727.8125*pow(time,5) + 2580645008.4063*pow(time,4) - 153336606.963*pow(time,3) + 3760479.4142*pow(time,2) - 3759.1468*time - 1450.7592);
      prop[SDOF_LOAD_F_X] = Sep_Fx1;
      prop[SDOF_LOAD_M_Z] = Sep_Mz1;
   }

wx1 = DT_OMEGA_CG(dt)[0];
wy1 = DT_OMEGA_CG(dt)[1];
wz1 = DT_OMEGA_CG(dt)[2];
Vx1 = DT_VEL_CG(dt)[0];
Vy1 = DT_VEL_CG(dt)[1];
Vz1 = DT_VEL_CG(dt)[2];

Domain *d10 = Get_Domain(1); /* for single phase flow*/
Thread *t_object10 = Lookup_Thread(d10, 138); /* you get Boundary_ID from the boundary condition panel in Fluent*/
real force10[ND_ND];
real moment10[ND_ND];
cg[0]=DT_CG(dt)[0];
cg[1]=DT_CG(dt)[1];
...
Compute_Force_And_Moment (d1,t_object10,cg,force10,moment10,TRUE);

real fx_fHH = force10[0]; /* force components of surface "Boundary_ID" */
real fy_fHH = force10[1];
real fz_fHH = force10[2];
real mx_fHH = moment10[0]; /* moment components */
real my_fHH = moment10[1];
real mz_fHH = moment10[2];

Domain *d11 = Get_Domain(1); /* for single phase flow*/
Thread *t_object11 = Lookup_Thread(d11, 144); /* you get Boundary_ID from the boundary condition panel in Fluent*/
real force11[ND_ND];
real moment11[ND_ND];

Compute_Force_And_Moment (d11,t_object11,cg,force11,moment11,TRUE);

real fx_NHH = force11[0]; /* force components of surface "Boundary_ID" */
real fy_NHH = force11[1];
real fz_NHH = force11[2];
real mx_NHH = moment11[0]; /* moment components */
real my_NHH = moment11[1];
real mz_NHH = moment11[2];

Domain *d12 = Get_Domain(1); /* for single phase flow*/
Thread *t_object12 = Lookup_Thread(d12, 146); /* you get Boundary_ID from the boundary condition panel in Fluent*/
real force12[ND_ND];
real moment12[ND_ND];

Compute_Force_And_Moment (d12,t_object12,cg,force12,moment12,TRUE);

real fx_btHH = force12[0]; /* force components of surface "Boundary_ID" */
real fy_btHH = force12[1];
real fz_btHH = force12[2];
real mx_btHH = moment12[0]; /* moment components */
real my_btHH = moment12[1];
real mz_btHH = moment12[2];

Domain *d13 = Get_Domain(1); /* for single phase flow*/
Thread *t_object13 = Lookup_Thread(d13, 141); /* you get Boundary_ID from the boundary condition panel in Fluent*/
real force13[ND_ND];
real moment13[ND_ND];

Compute_Force_And_Moment (d13,t_object13,cg,force13,moment13,TRUE);

real fx_btEHH = force13[0]; /* force components of surface "Boundary_ID" */
real fy_btEHH = force13[1];
real fz_btEHH = force13[2];
real mx_btEHH = moment13[0]; /* moment components */
real my_btEHH = moment13[1];
real mz_btEHH = moment13[2];

Fx1=fx_fHH+fx_btHH+fx_btEHH+fx_NHH;
Fy1=fy_fHH+fy_btHH+fy_btEHH+fy_NHH;
Fz1=fz_fHH+fz_btHH+fz_btEHH+fz_NHH;
Mx1=mx_fHH+mx_btHH+mx_btEHH+mx_NHH;
My1=my_fHH+my_btHH+my_btEHH+my_NHH;
Mz1=mz_fHH+mz_btHH+mz_btEHH+mz_NHH;
}

DEFINE_REPORT_DEFINITION_FN(Forcex_HH)
{
return Fx1;
}

DEFINE_REPORT_DEFINITION_FN(Forcey_HH)
{
return Fy1;
}
DEFINE_REPORT_DEFINITION_FN(Forcez_HH)
{
return Fz1;
}
DEFINE_REPORT_DEFINITION_FN(Momentx_HH)
{
return Mx1;
}
DEFINE_REPORT_DEFINITION_FN(Momenty_HH)
{
return My1;
}
DEFINE_REPORT_DEFINITION_FN(Momentz_HH)
{
return Mz1;
}

DEFINE_REPORT_DEFINITION_FN(Velx_HH)
{
return Vx1;
}

DEFINE_REPORT_DEFINITION_FN(Vely_HH)
{
return Vy1;
}

DEFINE_REPORT_DEFINITION_FN(Velz_HH)
{
return Vz1;
}

DEFINE_CG_MOTION(Follower_Fluid_HH,dt,vel,omega,time,dtime)
{
NV_S (vel, = , 0.0);
NV_S (omega, = , 0.0);
vel[0]=0;
